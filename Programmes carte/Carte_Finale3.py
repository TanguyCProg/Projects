...
def encadrer(indexes, temp):
    ...
    ind_data = np.array(ind_data, dtype = np.object)
    data = np.array(list(ind_data[:,2]),dtype = np.float16)
...
for Pays in liste_Pays:
    ...
    coords, temp = get_data(date, long_min, long_max, lat_min, lat_max)
    presence = np.array(presence,dtype = np.int16)
    t2 = time()
    ...
    for model in liste_models:
        compteur_g2 += 1
        indexes = []
        temperatures = np.zeros((n,n,7),dtype=np.float16)
                ...
                if not([delta_i1, delta_j1] in liste_deltas):
                    ...
                    for i2 in range(points[i1,j1,0],points[i1+1,j1,0]):
                        for j2 in range(points[i1,j1,1],points[i1,j1+1,1]):
                            ...
                            ratio2 /= somme_ratios
                            ratio3 /= somme_ratios
                            liste_ratio[num].extend(np.array([ratio0, ratio1, ratio2, ratio3]*7,dtype = np.float16))
                    ...
                    compteur = 0
                    temp0 = np.zeros((delta_i1, delta_j1,7),dtype = np.float16)
                    temp1 = np.zeros((delta_i1, delta_j1,7),dtype = np.float16)
                    temp2 = np.zeros((delta_i1, delta_j1,7),dtype = np.float16)
                    temp3 = np.zeros((delta_i1, delta_j1,7),dtype = np.float16)
                    temp0[:,:] = temp_mat[i1, j1]
                    temp1[:,:] = temp_mat[i1+1, j1]
                    temp2[:,:] = temp_mat[i1+1, j1+1]
                    temp3[:,:] = temp_mat[i1, j1+1]
