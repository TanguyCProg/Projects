...
n = 3000
#def Create_map(n):#Create_map(n,lis):
t0 = t.time()
def aff_carte():
...
def point_bordure(ip,jp,i,j, last_element, element):
    if i == ip:
        bordure_lat = []
...
def points_coins(entree,sortie, pixel):
    ...
    direction_coin = []
    for point in entree:
        k,pos_e,l_pos_s = entree_a_sortie(pixel[0], pixel[1],point,sortie)
        sortie.remove(sortie[k]) #sortie[k] vient d'être associée à une entrée, on ne doit plus la prendre en compte pour les prochains calculs
...
#liste_entree = [[[] for _ in range(n)] for _ in range(n)]
#liste_sortie = [[[] for _ in range(n)] for _ in range(n)]
t2 = t.time()
...
for element in lf[1:]:
    c += 1
    i = classage1(element, i)
    j = classage2(element, j)
    if i != ip or j != jp:
        res = point_bordure(ip,jp,i,j,last_element,element)
        if len(liste_enc) != 0:
            liste_enc[pos][1] = c + 20
        if len(res) == 3:
            ic,jc= classage(res[0],res[1])
            if [i,j] in liste_pat:
                pos_c = liste_pat.index([i,j])
                liste_enc[pos_c][1] = c + 20
            else:
                liste_pat.append([i,j])
                liste_enc.append([c-5,c+1])
            presence[ic][jc] = 2
        if [i,j] in liste_pat:
            pos = liste_pat.index([i,j])
            liste_enc[pos][1] = c + 20
        else:
            pos = -1
            liste_pat.append([i,j])
...
i_fin = i
j_fin = j
presence[i_init][j_init] = 3
presence[i_fin][j_fin] = 3


tot = len(liste_pat)
c = 0
for pixel, enc in zip(liste_pat[:-1],liste_enc[:-1]):
    c += 1
    if c % 90 == 0:
        print((c/tot)*100)
    entrees = []
    ...
    for element in lf[1:]:# lf[enc[0]:enc[1]+1]:
        i = classage1(element, i)
        j = classage2(element, j)

        
        
        if b_h <= i <= b_b and b_g <= j <= b_d:
            
            if Last and (i != ip or j != jp):
                res = point_bordure(ip,jp,i,j,last_element,element)
                
                if len(res) != 3:
                    
                    if pixel == [i,j]: #Le changement traité  est bien en rapport avec le pixel
                        entrees.append(res[1])
                    elif pixel == [ip,jp]:
                        sorties.append(res[0])
                        
                else:

                    ic,jc= classage(res[0],res[1])
                    #3 cas: Pixel = i, Pixel = ip, Pixel = ic
                    if pixel == [i,j]:
                        entrees.append(res[1])
                    elif pixel == [ip,jp]:
                        sorties.append(res[0])
                    elif pixel == [ic,jc]:
                        entrees.append(res[0])
                        sorties.append(res[1]) 
                        
            Last = True # Le point ip jp était dans la zone
    ...
    if len(entrees) != len(sorties):
        print(i,j,entrees,sorties)
    direction_coin = points_coins(entrees,sorties,pixel)
    i, j = pixel[0], pixel[1]
    for k in direction_coin:
        if k == 0:
...
##            liste_sortie[ip][jp].append(res[0])
##            liste_entree[i][j].append(res[1])
##
##            if len(res) == 3:
##                ic,jc= classage(res[0],res[1])
##                liste_entree[ic][jc].append(res[0])
##                liste_sortie[ic][jc].append(res[1])
##                presence[ic][jc] = 2
##                     
##            presence[ip][jp] = 2
##                
...
##    dir3 = [[0 for _ in range(n)] for _ in range(n)] # pos des coins
##    for i in range(n):
##        for j in range(n):
##            if presence[i][j] == 2:
##                direction_coin = points_coins(i,j)
##                dir3[i][j] = direction_coin
##
##                for k in dir3[i][j]:
##                    if k == 0:
##                        points = [[i-1,j], [i-1, j-1], [i, j-1]]
...
##                
t6 = t.time()
print("Recherche des coins : ",t6-t4)
